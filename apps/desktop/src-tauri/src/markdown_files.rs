...
fn is_markdown_history_file(path: &Path) -> bool {
    path.extension()
        .and_then(|extension| extension.to_str())
        .is_some_and(|extension| {
            matches!(
...
fn markdown_history_root(app: &tauri::AppHandle) -> Result<PathBuf, String> {
    app.path()
        .app_data_dir()
...
fn write_markdown_file_with_history_root(
    root: &Path,
    path: String,
    contents: String,
) -> Result<(), String> {
...
fn write_markdown_file_with_optional_history_root(
    root: Option<&Path>,
    path: String,
    contents: String,
