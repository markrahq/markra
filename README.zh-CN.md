...
- [主题自定义与 Typora 兼容说明](packages/app/src/themes/README.md)
...
| 自定义主题 | 主题文件夹、独立浅色/深色 CSS 文件、手动刷新，以及 Typora 基础兼容 | 自定义 CSS 编辑、导入/导出，以及 Typora 基础兼容 |
...
- 内置配色或限定作用域的自定义 CSS，浅色/深色分别配置，支持 CSS 导入/导出/重置。
- 桌面版可选择主题文件夹，或使用带可编辑模板的默认目录；目录和文件选择在重启后保留，手动刷新可应用到所有窗口。文件丢失或无法读取时，回退到设置中保存的 CSS。
- Typora 主题基础兼容：自动适配配色、本机已安装字体、标题字号/字重/字距、链接、代码字体/背景和常见代码高亮，保留原始 CSS。
- 通过 `--editor-code-font-family` 自定义行内代码和围栏代码块的字体。
- 导出为独立 HTML 或 PDF，控制页面、边距和元数据。导出使用独立样式，不跟随编辑器主题。

桌面版主题文件的使用方式：

1. 打开 **设置 → 外观**，通过 **选择主题文件夹** 指定已有 CSS 目录，或保留默认位置。点击 **打开主题文件夹** 可打开当前目录。
2. 放入 CSS 文件，或复制并修改默认目录中的 `starter-light.css`、`starter-dark.css` 模板。
3. 开启 **自定义主题**，分别选择浅色和深色的主题文件。
4. 在外部编辑并保存文件后，点击 **刷新主题**，将修改应用到所有窗口。

桌面版和 Web 版也可以在同一设置区域使用 **导入 CSS**。支持最大 **1 MiB** 的 UTF-8 CSS 文件。主题文件保存在本机，不包含在可移植配置备份中。

一次使用一个目录。切换到不同目录后，需要重新选择其中的主题文件；点击 **恢复默认文件夹** 可回到 Markra 的默认目录。自定义目录只读取，不会移动文件或写入模板。

Typora 兼容以基础外观为范围：复杂布局、条件规则、主题附带字体/图片和导入的样式表暂不处理，设置中会显示提示。完整变量和兼容范围见[主题说明](packages/app/src/themes/README.md)。
